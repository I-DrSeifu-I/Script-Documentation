...
#$NewNicName = Read-Host "`nEnter the NIC interface name - Network interface card name must be unique"
    ...
    $loggingNIC = "C:\Scripts\logging\$VMName-log($date).txt"
    ...
    if ( (Get-AzVM -Name $VMName -status).PowerState -eq "VM running" ) {
        ...
        "Stop and deallocate the VM: $VMName" | Out-File -FilePath $loggingNIC -Append
    }
    ...
    $NIC = New-AzNetworkInterface -Name $NewNicName -ResourceGroupName $resourceGroup `
        ...
        "Creating the new virtual Network interface: $NewNicName" | Out-File -FilePath $loggingNIC -Append
    ...
    "Removing the old NIC interface: $($oldNicName) from the VM: $VMName" | Out-File -FilePath $loggingNIC -Append
    ...
    "Adding the new network adapter interface to: $VMName" | Out-File -FilePath $loggingNIC -Append
    ...
    "Deleting the old NIC interface: $($oldNicName)" | Out-File -FilePath $loggingNIC -Append
    ...
    "Updating the new NIC settings for: $NewNicName" | Out-File -FilePath $loggingNIC -Append
    ...
    If ($NIC.IpConfigurations.PrivateIpAddress -ne $vNIC.IpConfigurations.PrivateIpAddress) {
        Set-AzNetworkInterfaceIpConfig -NetworkInterface $NIC -Name $NIC.IpConfigurations.Name `
            ...
            "Setting the new NIC settings for: $NewNicName" | Out-File -FilePath $loggingNIC -Append
    }
    ...
    If ($PIpName) {
        ...
        Set-AzNetworkInterface -NetworkInterface $NIC | Out-Null
        "Associating the public IP with: $VMName" | Out-File -FilePath $loggingNIC -Append
    }
}

#! Start the VMName
Write-Verbose "Start the VM: $VMName, please wait..."
"Starting VM: $VMName" | Out-File -FilePath $loggingNIC -Append
Start-AzVM -Name $VMName -ResourceGroupName $resourceGroup -Confirm:$false | Out-Null
"New NIC has been built out for $VMName" | Out-File -FilePath $loggingNIC -Append
